...
4-signal: MR(0) + MR_secondary(1) + Trend(2) + Breakout_High(3)
Sort: (prio, drop_depth, RSI) — deeper drops first within same priority
...
def compute_all_signals(stock_list, g):
    """
    Returns list of (stock, prio, drop_5d, rsi6, close_price).
    v8: includes MR(main=0, sec=1), Trend(2), Breakout_High(3).
    """
    n = 30
    ...
    for stock in stock_list:
        ...
        if drop_5d < -0.10 and rsi6 < 20:
            candidates.append((stock, 0, drop_5d, rsi6, cc))
...
def _compute_trend_signals(stock_list, cd):
    n = 25
    df_close = history(n, unit='1d', field='close', security_list=stock_list,
                       df=True, skip_paused=False, fq='pre')
    ...
    for stock in stock_list:
        ...
        if c > ma20 and c1 < ma20 and v > PARAMS['trend_vol_mult'] * v20:
            ...
            result.append((stock, 2, -0.05, 50, c))
    return result

def _compute_breakout_signals(stock_list, cd):
    n = 25
    df_close = history(n, unit='1d', field='close', security_list=stock_list,
                       df=True, skip_paused=False, fq='pre')
    df_high = history(n, unit='1d', field='high', security_list=stock_list,
                      df=True, skip_paused=False, fq='pre')
    if df_close is None or df_close.empty: return []
    result = []
    for stock in stock_list:
        try:
            close_s = df_close[stock].dropna(); high_s = df_high[stock].dropna()
        ...
        if c > h20 and ma20 > ma60:
            ...
            if d.paused or d.is_st: continue
            if d.high_limit > 0 and d.last_price >= d.high_limit: continue
            result.append((stock, 3, -0.05, 50, c))
    return result
...
def daily_handle(context):
    ...
    if len(candidates) > 0 or len(g.holdings) == 0 or g.bar_index % 60 == 0:
        log.info('[%s] regime=%s holdings=%d cash=%.0f total=%.0f signals=%d' %
                 (today, g.regime, len(g.holdings), cash, total_value, len(candidates)))
